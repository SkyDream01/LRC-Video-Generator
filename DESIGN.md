...
4. 评分: score = 0.5·饱和度 + 0.3·亮度适中 + 0.2·簇面积
    ...
    │    辅色: 与主色色相差 ≥ 30° 的次高分簇
    └─ 歌词色/描边色: 始终使用 lyric_style 中的所选颜色，不受自动取色影响
5. 手动模式下用户可覆盖动画配色；勾选「自动」时每次换封面重新提取动画主色/辅色
```

歌词默认是白字深描边，包括默认色在内的所有选择均按原值渲染，确保颜色按钮与预览、导出一致。自动取色只驱动动画配色。
...
导出期间状态栏显示渲染时间、预计剩余时间和渲染倍数（已渲染视频秒数 / 实际耗时，2× 表示两倍实时速度），每 500ms 用单调时钟刷新。耗时从开始导出计起，包含准备与重试；剩余时间按当前编码尝试的平均速度估算，无样本显示「估算中」，编码器回退时重新估算。帧提交完毕后显示「正在封装」，收到成功信号才完成；完成弹窗显示总耗时与平均倍数。导出期间暂时隐藏预览帧率和音频引擎标签，为统计信息留出空间。
