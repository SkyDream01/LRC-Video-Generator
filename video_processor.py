...
def get_ffmpeg_probe_path(ffmpeg_path_str: str) -> str:
    ...
    if ffmpeg_path_str == 'ffmpeg':
        if shutil.which('ffprobe'):
            return 'ffprobe'
        if sys.platform == 'win32' and shutil.which('ffprobe.exe'):
            return 'ffprobe.exe'
    ...
    # 2. 如果指定了绝对路径但找不到同级ffprobe，尝试系统查找
...
def _build_filter_complex(params: VideoGenParams, lrc_data: List, is_preview: bool) -> str:
    ...
    params.logger.status_update(f"构建滤镜图 (背景: {params.background_anim}, 歌词: {params.text_anim})...")
    ...
    # 动态传递分辨率参数
    try:
        bg_filter_str = bg_func(W=W, H=H, FPS=FPS, duration=params.duration)
    except TypeError:
         # 兼容旧接口（虽然下面我们更新了animations.py，但保留容错是好的习惯）
         bg_filter_str = bg_func(duration=params.duration)
    ...
    # 传递 FPS 
    try:
        cover_filter_str = cover_func(duration=params.duration, fps=FPS)
    except TypeError:
        cover_filter_str = cover_func(duration=params.duration)
...
def _run_ffmpeg_process(command: List[str], logger, duration: float = 0):
    ...
    display_command = []
    for arg in command:
        if arg.startswith('filter_complex_script'):
            display_command.append(arg)
        elif len(str(arg)) > 200:
             display_command.append(str(arg)[:50] + "...")
        else:
            display_command.append(str(arg))
            
    logger.status_update(f"执行FFmpeg命令: {' '.join(display_command)}")
...
def _process_media(params: VideoGenParams, is_preview: bool = False):
    ...
    try:
        ...
        logger.status_update("正在解析LRC文件...")
        ...
        if is_preview:
            ...
            for key, val in hw_configs.items():
                if key in params.hw_accel:
                    ...
                    logger.status_update(f"使用硬件加速配置: {key}")
                    break
